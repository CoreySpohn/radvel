...
@cython.boundscheck(False)
def calc_rv_forward_model(cnp.ndarray[DTYPE_t, ndim=1] t, param_obj, vector, planet_num=None):

    cdef cnp.ndarray[DTYPE_t, ndim=2] params = vector.vector
    ...
    cdef cnp.ndarray[DTYPE_t, ndim=1] rv = np.zeros(ntimes)
...
@cython.boundscheck(False)
def model_call_c(cnp.ndarray[DTYPE_t, ndim=1] t, param_obj, vector, double time_base, int jit_index, int gamma_index, *args, **kwargs):

    cdef cnp.ndarray[DTYPE_t, ndim=2] params = vector.vector
    basis = param_obj.basis.name.encode("utf-8")
    cdef int nplan = param_obj.num_planets
    cdef int ntimes = len(t)
    cdef cnp.ndarray[DTYPE_t, ndim=1] model_values = np.zeros(ntimes)
...
@cython.boundscheck(False)
def residuals_c(cnp.ndarray[DTYPE_t, ndim=1] t, cnp.ndarray[DTYPE_t, ndim=1] rv, cnp.ndarray[DTYPE_t, ndim=1] rv_err, 
    ...
    cdef int ntimes = len(t)
    cdef int specific_planet = 0
    cdef cnp.ndarray[DTYPE_t, ndim=1] res = np.zeros(ntimes)
    cdef cnp.ndarray[DTYPE_t, ndim=1] rv_err2 = np.zeros(ntimes)
    cdef cnp.ndarray[DTYPE_t, ndim=1] sum_sig_quad = np.zeros(ntimes)
...
@cython.boundscheck(False)
def logprob_c(cnp.ndarray[DTYPE_t, ndim=1] t, cnp.ndarray[DTYPE_t, ndim=1] rv, cnp.ndarray[DTYPE_t, ndim=1] rv_err, 
        param_obj, vector, double time_base, int jit_index, int gamma_index):
